...
template <typename T>
bool autosorted_dynamic_array<T>::insert(T val, int64_t index){ /// Consider if adding boundary checks would be good. Propably
...
template <typename T>
void autosorted_dynamic_array<T>::append(T val){
    insert(val, last_index);
}

template <typename T>
void autosorted_dynamic_array<T>::prepend(T val){
    insert(val, 0);
}
...
template <typename T>
void autosorted_dynamic_array<T>::force_shrink(int64_t new_size){
